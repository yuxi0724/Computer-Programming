                     ...
                     int *&productCoef, int *&productExpon, int &productSize )
{
   ...
   productCoef = new int[bufferSize * multiplierSize]();
   productExpon = new int[bufferSize * multiplierSize]();
   // if( multiplicand != 0 && multiplier != 0 )
   if( multiplicandSize != 0 && multiplierSize != 0 )
   {
       int buf=0;
       while (buf != multiplierSize)
       {
           for (int i = 0; i < bufferSize; i++)
           {
               bufferExpon[i] = multiplicandExpon[i] + multiplierExpon[buf];
               bufferCoef[i] = multiplicandCoef[i] * multiplierCoef[buf];
           }
           if (buf == 0)
           {
               for (int i = 0; i < bufferSize; i++)
               {
                   productCoef[i] = bufferCoef[i];
                   productExpon[i] = bufferExpon[i];
                   productSize++;
               }
           }
           else
           {
               for (int i = 0; i < bufferSize; i++)
               {
                   ...
                   {
                       if (productExpon[j] == bufferExpon[i])
                       {
                           productCoef[j] += bufferCoef[i];
                           break;
                       }
                       if (j == productSize - 1)
                       {
                           productExpon[productSize] = bufferExpon[i];
                           productCoef[productSize] = bufferCoef[i];
                           productSize++;
                           break;
                       }
                   }
   ...
   for (int i = 0; i <= pcs; i++)
   {
       if (productCoef[i] == 0)
       {
           if (i != pcs)
           {
               for (int j = i; j < pcs; j++)
               {
                   productCoef[j] = productCoef[j + 1];
                   productExpon[j] = productExpon[j + 1];
               }
           }
           productSize--;
   ...
   {
       int maxE = productExpon[i], maxC = productCoef[i],max=i;
       for (int j = i+1; j < productSize; j++)
       {
           if (productExpon[j] > maxE)
           {
               maxE = productExpon[j];
               maxC = productCoef[j];
               max = j;
       ...
       productCoef[max] = productCoef[i];
       productExpon[max] = productExpon[i];
       productCoef[i] = maxC;
       productExpon[i] = maxE;
   }
    ...
    int*& remainderCoef, int*& remainderExpon, int& remainderSize)
{
    ...
    quotientSize = 0;
     ...
     bufferCoef = new int[bufferSize]();
     bufferExpon = new int[bufferSize]();
    ...
    while (remainderSize > 0 && remainderExpon[0] >= divisorExpon[0])
    {
        ...
        for (int i = 0; i < bufferSize; i++)
        {
            bufferExpon[i] = monomialExpon[0] + divisorExpon[i];
            bufferCoef[i] = monomialCoef[0] * divisorCoef[i];
        }

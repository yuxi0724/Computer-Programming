...
void subtraction( int *minuend, int *subtrahend, int &minuendDegree, int subtrahendDegree )
{
    for (int i = subtrahendDegree ; i >= 0; i--)
    {
               ...
               int dividendDegree, int divisorDegree, int quotientDegree, int &remainderDegree )
{
    ...
    for (int i = 0; i <= quotientDegree; i++)
        quotient[i] = 0;
    int monoD = quotientDegree;
    while (remainderDegree >= divisorDegree && remainderDegree > 0)
    {
        quotient[monoD] = remainder[remainderDegree] / divisor[divisorDegree];
        int* mono = new int[monoD + 1]();
        for (int i = 0; i <= monoD; i++)
            mono[i] = 0;
        mono[monoD] = quotient[monoD];
        int bufD = monoD + divisorDegree;
        int* buf = new int[bufD + 1]();
        multiplication(mono, divisor, buf, monoD, divisorDegree, bufD);
        subtraction(remainder, buf, remainderDegree, bufD);
        monoD=remainderDegree-divisorDegree;

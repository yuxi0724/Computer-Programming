   ...
   Term *multiplier, int multiplierSize, Term *&product, int &productSize )
{
   ...
   Term *buffer = new Term[ bufferSize ]();
   product = new Term[bufferSize * multiplierSize]();
   ...
   if (multiplicandSize != 0 && multiplierSize != 0)
   {
       ...
       while (buf != multiplierSize)
       {
           ...
           {
               buffer[i].expon = multiplicand[i].expon + multiplier[buf].expon;
               buffer[i].coef = multiplicand[i].coef * multiplier[buf].coef;
           }
               ...
               Term *&quotient, int &quotientSize, Term *&remainder, int &remainderSize )
{
   ...
   while (remainderSize > 0 && remainder[0].expon >= divisor[0].expon)
   {
       ...
       for (int i = 0; i < bufferSize; i++)
       {
           buffer[i].expon = monomial[0].expon + divisor[i].expon;
           buffer[i].coef = monomial[0].coef * divisor[i].coef;
       }

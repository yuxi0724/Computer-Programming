...
void multiplication( const Polynomial &multiplicand, const Polynomial &multiplier, Polynomial &product )
{
   ...
   int bufferSize = multiplicand.size;
   Polynomial buffer ;
   buffer.terms= new Term[bufferSize]();
   buffer.size = multiplicand.size;
   product.size = buffer.size * multiplier.size;
   product.terms =new Term[product.size]();
   // if( multiplicand != 0 && multiplier != 0 )
   if( multiplicand.size != 0 && multiplier.size != 0 )
   {
       int buf = 0;
       while (buf != multiplier.size)
       {
           for (int i = 0; i < bufferSize; i++)
           {
               buffer.terms[i].expon = multiplicand.terms[i].expon + multiplier.terms[buf].expon;
               buffer.terms[i].coef = multiplicand.terms[i].coef * multiplier.terms[buf].coef;
           }
               ...
               Polynomial &quotient, Polynomial &remainder )
{
   ...
   while (remainder.size > 0 && remainder.terms[0].expon >= divisor.terms[0].expon)
   {
       ...
       {
           buffer.terms[i].expon = monomial.terms[0].expon + divisor.terms[i].expon;
           buffer.terms[i].coef = monomial.terms[0].coef * divisor.terms[i].coef;
       }
